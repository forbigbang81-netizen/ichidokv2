#!/usr/bin/env python3
"""
Process one batch of anigo.re slugs. Reads slugs from a file, processes each one,
saves results to a JSON file. Designed to be called once per batch (low memory).
"""
import json
import re
import sys
...
def process_anime(slug: str) -> dict | None:
    ...
    if prefix not in ("ani", "mal"):
        # For /s-N/ patterns, only fetch up to 20 episodes to avoid timeouts
        embeds = {}
        for num, url in info["episodes"][:20]:
            h = fetch(url)
            if h:
                ...
                if e:
                    embeds[num] = e
        out["embeds"] = embeds
        # Update ep_count_actual to match what we have
        out["ep_count_actual"] = len(embeds) if embeds else len(info["episodes"])
    return out


def main():
    if len(sys.argv) != 3:
        print(f"Usage: {sys.argv[0]} <batch_file> <output_file>", file=sys.stderr)
        sys.exit(1)
    batch_file = sys.argv[1]
    output_file = sys.argv[2]
    
    slugs = [s.strip() for s in open(batch_file).readlines() if s.strip()]
    print(f"Processing {len(slugs)} slugs from {batch_file}", flush=True)
    ...
    for i, slug in enumerate(slugs, 1):
        ...
        if data:
            results.append(data)
            print(f"[{i}/{len(slugs)}] OK  {slug} ({data['ep_count_actual']} eps)", flush=True)
        else:
            print(f"[{i}/{len(slugs)}] SKIP {slug}", flush=True)
    
    Path(output_file).write_text(json.dumps(results, indent=2), encoding="utf-8")
    print(f"Saved {len(results)} entries to {output_file}", flush=True)
